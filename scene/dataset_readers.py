...
from ct.astra_ct import astra_ct
...
def readCameras(camera_file, datapath):
    ...
    if (mask_run <= 0).any():
        num_invalid = (mask_run <= 0).sum()
        print(f"Warning: mask_run contains {num_invalid} non-positive values. Clipping to {epsilon}.")
        mask_run = np.clip(mask_run, epsilon, None)

    if (fill_run <= 0).any():
        num_invalid = (fill_run <= 0).sum()
        print(f"Warning: fill_run contains {num_invalid} non-positive values. Clipping to {epsilon}.")
        fill_run = np.clip(fill_run, epsilon, None)

    proj = np.log(mask_run) - np.log(fill_run)

    if np.isnan(proj).any():
        num_nan = np.isnan(proj).sum()
        print(f"Error: proj contains {num_nan} NaN values after log subtraction!")
        proj = np.nan_to_num(proj, nan=0.0)

    if np.isinf(proj).any():
        num_inf = np.isinf(proj).sum()
        print(f"Warning: proj contains {num_inf} Inf values after log subtraction!")
        proj = np.nan_to_num(proj, posinf=proj[~np.isinf(proj)].max(), neginf=0.0)

    proj = np.clip(proj, 0, proj.max())

    print(f"DSA projection statistics:")
    print(f"  Shape: {proj.shape}")
    print(f"  Range: [{proj.min():.6f}, {proj.max():.6f}]")
    print(f"  Mean: {proj.mean():.6f}, Std: {proj.std():.6f}")
    ...
    for idx in tqdm(range(0, N_views), desc='camera loading'):
        ...
        extr = get_extrinsic(PrimaryAngle, SecondaryAngle, sad)
        R = extr[:3, :3]
        T = extr[:3, 3]
        ...
        cam_info = CameraInfo(
            uid=uid, timestamp=timestamp, R=R, T=T,
            PrimaryAngle=PrimaryAngle, SecondaryAngle=SecondaryAngle,
            sad=sad, sid=sid, FovY=FovY, FovX=FovX,
            sx=sx, sy=sy, near=near, far=far,
...
def vol_initializor(recon_vol, recon_args, init_args, type='fdk'):
    ...
    coords_matrix = make_coords(
        recon_args['volume_resolution'],
        recon_args['volume_phy'],
        recon_args['volume_origin']
    )
...
def readSceneInfo(datapath, outpath, train_views, init_args, loaded_iter):
    ...
    if not loaded_iter:
        if init_args['fdk_initial']:
            ...
            CT_reconstructor = astra_ct(train_cam_infos, recon_args)
            FDK_recon = CT_reconstructor.fdk(
                CT_reconstructor.projs,
                CT_reconstructor.PrimaryAngles
            )
            FDK_recon = np.clip(FDK_recon, a_min=0, a_max=FDK_recon.max())
            pcd = vol_initializor(FDK_recon, recon_args, init_args, type='fdk')
            sitk.WriteImage(
                sitk.GetImageFromArray(FDK_recon.transpose(2, 1, 0)),
                fdk_file
            )
        else:
